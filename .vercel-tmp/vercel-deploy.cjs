 ...
 * Usage: bun deploy.cjs [project-path] [options]
...
const ALLOWED_COMMANDS = new Set(['vercel', 'npm', 'pnpm', 'yarn', 'bun']);
function log(msg) {
  console.error(msg);
}
...
function detectPackageManager(projectPath) {
  ...
  if (fs.existsSync(path.join(projectPath, 'bun.lock'))) return 'bun';
...
function runBuildIfNeeded(projectPath) {
  ...
  if (!pkgManager) {
    log('Error: No package manager found (npm/pnpm/yarn/bun)');
    process.exit(1);
  }
  ...
  if (!fs.existsSync(nodeModulesPath)) {
    ...
    log('');
    const installArgs = pkgManager === 'bun' ? ['install'] : pkgManager === 'yarn' ? [] : ['install'];
    try {
  ...
  // Run build command
  const buildArgs = pkgManager === 'npm' ? ['run', 'build'] : pkgManager === 'bun' ? ['run', 'build'] : ['build'];
...
function main() {
  ...
  if (!checkLoginStatus()) {
    ...
    log('Please run login first to authorize');
    process.exit(1);
  }
  log('');
  // Default to frontend directory for this project
  const projectPath = checkProject('./frontend');
