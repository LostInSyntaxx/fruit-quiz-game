...
const ALLOWED_COMMANDS = new Set(['node', 'npm', 'pnpm', 'yarn', 'bun', 'vercel']);
function log(msg) {
console.error(msg);
}
...
function checkNode() {
if (!commandExists('node') && !commandExists('bun')) {
log('Error: Node.js or Bun is not installed');
log('Please install Node.js (v16+) or Bun first: https://nodejs.org/ or https://bun.sh/');
process.exit(1);
}
const version = getCommandOutput('bun', ['--version']) || getCommandOutput('node', ['-v']);
log(`Detected runtime: ${version}`);
}
